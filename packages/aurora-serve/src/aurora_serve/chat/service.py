...
from aurora_core.session import SessionManager, SessionMessage, Session
from aurora_core.memory import MemoryManager, MemoryType
from aurora_core.hooks import HookManager, HookType
from aurora_core.context import ContextCompactor, ToolSearchManager, ContextMonitor
from aurora_core.subagents import SubagentManager, SubagentDefinition
from aurora_core.permissions import PermissionManager, PermissionMode
...
from aurora_core.status import CostTracker
...
class EnhancedChatService:
    ...
    def __init__(
        ...
        if project_path:
            project_settings_file = str(Path(project_path) / ".aurora" / "settings.json")
        ...
        project_agents_path = str(Path(project_path) / ".aurora" / "agents") if project_path else None
    ...
    def _get_llm(self, req: ChatRequest) -> Union[OpenAILLM, AnthropicLLM]:
        ...
        if req.model_config_field:
            ...
            cfg = LLMConfig(
                ...
                extra={"api_version": req.model_config_field.api_version}
                if req.model_config_field.api_version
                else {},
            )
    ...
    def _effective_model_type(self, req: ChatRequest) -> str:
        ...
        if req.model_config_field:
            if "kimi.com/coding" in (req.model_config_field.base_url or ""):
                return "openai"
            if req.model_config_field.model_type in {"daemon", "cli"}:
                return "daemon"
...
<body>
  <main class="shell">
    ...
    async def chat(
        ...
        tools = None
        if model_type not in ("anthropic", "daemon", "cli", "google"):
    ...
    def get_status(self) -> dict:
        ...
        """
        from aurora_core.status.models import ModelInfo, StatusData, WorkspaceInfo, GitInfo
