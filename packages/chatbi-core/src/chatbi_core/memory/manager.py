"""Memory Manager for ChatBI - handles persistent memory."""
...
from chatbi_core.memory.schema import MemoryEntry, MemoryType


class MemoryManager:
    """Manages persistent memory following Claude Code patterns.

    Memory is stored in Markdown files under ~/.chatbi/memory/
    and project-level .chatbi/memory/
    """
    ...
    def __init__(
        ...
        self.global_path = Path(global_path or os.path.expanduser("~/.chatbi/memory"))
        self.project_path = Path(project_path) if project_path else None

        self.global_path.mkdir(parents=True, exist_ok=True)
        if self.project_path:
            memory_path = self.project_path / ".chatbi" / "memory"
    ...
    def _load_memories(self) -> None:
        ...
        if self.project_path:
            project_memory = self.project_path / ".chatbi" / "memory"
    ...
    def save_memory(self, entry: MemoryEntry) -> str:
        ...
        if entry.type == MemoryType.USER:
            ...
            save_path = self.project_path / ".chatbi" / "memory" if self.project_path else self.global_path
    ...
    def _update_memory_index(self) -> None:
        ...
        lines: List[str] = ["# ChatBI Memory Index\n"]
        ...
        if self.project_path:
            project_index = self.project_path / ".chatbi" / "memory" / "MEMORY.md"
