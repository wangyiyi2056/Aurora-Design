...
import * as Constants from '@/features/construct/knowledge/lib/constants'
import { useGraphStore, RawGraph, RawNodeType, RawEdgeType } from '@/features/construct/knowledge/stores/graph'
import { toast } from 'sonner'
import { getSubgraph } from '@/services/knowledge-v2'
...
const fetchGraph = async (knowledgeName: string, label: string, maxDepth: number, maxNodes: number) => {
  let rawData: any;
  ...
  try {
    ...
    rawData = await getSubgraph(knowledgeName, queryLabel, maxDepth, maxNodes);
  } catch (e) {
  ...
  if (rawData) {
    ...
    if (!validateGraph(rawGraph)) {
      console.warn('Graph data has validation warnings (e.g. missing properties or edges pointing to non-existent nodes), but will attempt to render valid parts anyway.')
    }
...
const createSigmaGraph = (rawGraph: RawGraph | null) => {
  ...
  for (const rawEdge of rawGraph?.edges ?? []) {
    ...
    try {
      ...
      console.warn("Failed to add edge", rawEdge, e);
    }
  }
...
const useLightragGraph = (knowledgeName: string) => {
  ...
  useEffect(() => {
    ...
    if (!isFetching && !useGraphStore.getState().graphDataFetchAttempted) {
      ...
      // 1. Always fetch graph (fetchGraph defaults label to '*')
      const dataPromise = fetchGraph(knowledgeName, currentQueryLabel, currentMaxQueryDepth, currentMaxNodes);
  ...
  useEffect(() => {
    ...
    const handleNodeExpand = async () => {
      ...
      try {
        ...
        const extendedGraph = await getSubgraph(knowledgeName, label, 2, 1000);
...
export { useLightragGraph, fetchGraph, createSigmaGraph }
